...
def make_dataset(
    num_sample: int = 10, prob_swap: float = 0.0, seed: int = 0
...
def make_dataframe(dataset: Optional[enn_base.BatchIterator] = None) -> pd.DataFrame:
...
def make_plot_data(
    experiment: supervised.BaseExperiment, num_sample: int
) -> pd.DataFrame:
...
def make_sample_plot(plot_df: pd.DataFrame, data_df: Optional[pd.DataFrame] = None):
    ...
    p = (
        ...
        + gg.geom_point(data=data_df, colour="black", size=5, stroke=2)
...
def make_mean_plot(plot_df: pd.DataFrame, data_df: Optional[pd.DataFrame] = None):
...
def colab_plots(experiment: supervised.BaseExperiment):
    ...
    make_sample_plot(plot_df[plot_df["sample"] < 12], dataframe).draw()
